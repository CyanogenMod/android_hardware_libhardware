...

    {
        ...
        while ((remaining_frames > 0) && (attempts < MAX_READ_ATTEMPTS)) {
            ...
            if (output_channels == 1 && input_channels == 2) {
                ...
                // use local temp buffer store the stereo pipe audio
                // the original buff may be only mono size
                saved_buff = buff;
                buff = (char*)rsxadev->routes[in->route_handle].resampler_buffer;
            }
#endif // ENABLE_CHANNEL_CONVERSION
            ...
            if (saved_buff) {
                // If both channel coversion and resampling are enabled, resampling process already
                // copied all data from local temp buffer to destination one.
                // If only channel conversion is enabled, it is necessary to copy data over here.
