...
#if defined(ENABLE_RESAMPLING) || defined(ENABLE_CHANNEL_CONVERSION)
    // Buffer used as temporary storage for data prior to returning data to the output
    // stream.
    int16_t processor_buffer[DEFAULT_PIPE_SIZE_IN_FRAMES];
#endif // ENABLE_RESAMPLING || ENABLE_CHANNEL_CONVERSION
} route_config_t;
                       ...
                       size_t bytes)
{
...

    {
        ...
        const size_t resampler_buffer_size_frames =
            sizeof(rsxadev->routes[in->route_handle].processor_buffer) /
                sizeof(rsxadev->routes[in->route_handle].processor_buffer[0]);
...
#if defined(ENABLE_CHANNEL_CONVERSION) || defined(ENABLE_RESAMPLING)
