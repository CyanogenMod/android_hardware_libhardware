...
typedef struct amplifier_module {
     ...
     *
     * Should only occur once on audio HAL initialization
     */
    int (*init)(struct amplifier_module *module);
     ...
     */
    int (*set_devices)(struct amplifier_module *module, int devices);

    /**
     * Notify amplifier module about current audio mode
     */
    int (*set_mode)(struct amplifier_module *module, audio_mode_t mode);

    /**
     * Notify amplifier module that an output stream has started
     */
    int (*stream_start)(struct amplifier_module *module,
            struct audio_stream_out *stream, bool offload);

    /**
     * Notify amplifier module that an output stream has stopped
     */
    int (*stream_standby)(struct amplifier_module *module,
            struct audio_stream_out *stream);

    /**
     * Shut down amplifier hardware
     */
    int (*shutdown)(struct amplifier_module *module);
} amplifier_module_t;
